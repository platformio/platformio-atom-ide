...
export default class ProjectExamplesStage extends BaseStage {
  ...
  static STORAGE_KEY = 'platformio-ide:project-examples';
  ...
  static DESTINATION_DIR = path.join(config.BASE_DIR, 'project-examples');
  ...
  async unpackExamples(targzPath) {
    ...
    if (utils.isDir(ProjectExamplesStage.DESTINATION_DIR)) {
      try {
  ...
  async check() {
    ...
    if (await this.isOutdatedExamples()) {
      if (utils.isFile(ProjectExamplesStage.ARCHIVE_CACHE_PATH)) {
        try {
