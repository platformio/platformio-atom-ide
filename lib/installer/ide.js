...
export async function checkIDEUpdates() {
  ...
  if (selected === 0) {
    try {
      ...
      console.error(err);
      atom.notifications.addError('Problem occured while upgrading PlatformIO IDE', {
        description: 'Please upgrade `platformio-ide` package manually (Settings > Updates)',
        dismissable: true
      });
    }
  }
...
function fetchLatestIDEVersion(branch) {
  return new Promise((resolve, reject) => {
    pioNodeHelpers.misc.processHTTPRequest(
      `https://raw.githubusercontent.com/platformio/platformio-atom-ide/${branch}/package.json`,
...
export async function reinstallIDE(useDevelop) {
  try {
    ...
    utils.notifyError('Problem has been occured while installing PlatformIO IDE.', err);
    return false;
  }
