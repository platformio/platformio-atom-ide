...
module.exports = {
  ...
  activate: function(state) {
    ...
    for (let target of ['Build', 'Upload', 'Clean']) {
      this.subscriptions.add(atom.commands.add('atom-workspace', `platformio-ide:trigger:${target.toLowerCase()}`, () => {
        ...
        const status = runAtomCommand(`platformio-ide:trigger:${target.toLowerCase()}-${p}`);
        if (!status) {
  ...
  consumeToolBar: function(toolBar) {
    ...
    this.toolBar.addButton({
      icon: 'check',
      callback: 'platformio-ide:trigger:build',
      tooltip: 'PlatformIO: Build'
    });

    this.toolBar.addButton({
      icon: 'arrow-right',
      callback: 'platformio-ide:trigger:upload',
      tooltip: 'PlatformIO: Upload'
    });

    this.toolBar.addButton({
      icon: 'trashcan',
      callback: 'platformio-ide:trigger:clean',
