...
module.exports = {
  ...
  activate: function(state) {
    ...
    for (let target of ['Build', 'Upload', 'Clean']) {
      this.subscriptions.add(atom.commands.add('atom-workspace', `platformio-ide:${target.toLowerCase()}`, () => {
        ...
        const status = runAtomCommand(`platformio-ide:build.${target.toLowerCase()}-${p}`);
        if (!status) {
  ...
  consumeToolBar: function(toolBar) {
    ...
    this.toolBar.addButton({
      icon: 'check',
      callback: 'platformio-ide:build',
      tooltip: 'PlatformIO: Build'
    });

    this.toolBar.addButton({
      icon: 'arrow-right',
      callback: 'platformio-ide:upload',
      tooltip: 'PlatformIO: Upload'
    });

    this.toolBar.addButton({
      icon: 'trashcan',
      callback: 'platformio-ide:clean',
