...
export default class PlatformIOBuildProvider {
  ...
  getNiceName() {
    return PlatformIOBuildProvider.title;
  }
  ...
  async settings() {
    ...
    for (const section of Object.keys(projectConf)) {
      ...
      if (!platform || !section.startsWith(PlatformIOBuildProvider.ENV_NAME_PREFIX)) {
        continue;
      }
      projectData.push({
        env: section.slice(PlatformIOBuildProvider.ENV_NAME_PREFIX.length),
        platform
      });
    }
    ...
    // base targets
    PlatformIOBuildProvider.baseTargets.forEach(target => {
...
class RunTarget {
  ...
  get _restoreTerminal() {
    ...
    return ['upload', 'program', 'uploadfs'].some(arg => this.args.includes(arg));
  }
  ...
  get postBuild() {
    return buildOutcome => {
      if (!buildOutcome || !this._terminalsToRestore) {
        return;
      }
      setTimeout(() => {
