...
import { debugProject } from './dbg.js';
import fs from 'fs';
import ini from 'ini';
...
class RunTarget {
  ...
  get _coreFirstProjectEnvName() {
    const content = fs.readFileSync(
      path.join(this._projectDir, 'platformio.ini'), 'utf-8');
    const projectConf = ini.parse(content);
    for (const section of Object.keys(projectConf)) {
      const platform = projectConf[section].platform;
      if (!platform || !section.startsWith(BuildProvider.ENV_NAME_PREFIX)) {
        continue;
      }
      return section.slice(BuildProvider.ENV_NAME_PREFIX.length);
    }
    return undefined;
  }
  ...
  get postBuild() {
    return buildOutcome => {
      ...
      if (this._coreTargetName === 'debug') {
        debugProject(
          this._projectDir, this._coreEnvName || this._coreFirstProjectEnvName);
      }
