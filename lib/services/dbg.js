...
export class ProjectDebug {
  ...
  static SERVER_ERROR_TITLE = 'Could not start Debug Server. Try to reconnect your device.';
      ...
      this.emitter.on('server-started', () => {
        ...
        delayedTimer = setTimeout(() => {
          if (!this._server_process) {
            atom.notifications.addError(ProjectDebug.SERVER_ERROR_TITLE, {
  ...
  startServer(options) {
    ...
    p.onWillThrowError(errorObject => {
      errorObject.handle();
      utils.notifyError(ProjectDebug.SERVER_ERROR_TITLE, new Error(errorObject.error));
      this.terminate();
    });
    return p;
  }
  ...
  launchDbg(options) {
    ...
    let gdb_commands = options.gdbinit || [];
    // fix Windows file path
    gdb_commands = gdb_commands.map(cmd => cmd.replace(/\\/g, '\\\\'));
    dbgService.debug({
      debugger: 'dbg-gdb',
