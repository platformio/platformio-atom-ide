      ...
      fs.readFile(this.platformioIniPath, (err, data) => {
        ...
        if (envs.length > 1) {
          for (let env of envs) {
            ...
            if ('espressif' !== config[envNamePrefix + env].platform) {
              envSettings = envSettings.filter((item) => item.args.indexOf('uploadfs') === -1);
            } else if ('atmelavr' !== config[envNamePrefix + env].platform) {
              envSettings = envSettings.filter((item) => item.args.indexOf('program') === -1);
            }
