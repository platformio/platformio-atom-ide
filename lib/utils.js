...
export function runCommand(cmd, args, callback, cacheValid = null) {
  let cache = null;
  let cacheKey = null;
  if (cacheValid) {
  ...
  console.debug('runCommand', cmd, args);
  const outputLines = [],
    errorLines = [];
  new BufferedProcess({
    command: cmd,
...
export function runPIOCommand(args, callback, cacheValid = null) {
  runCommand(
    'platformio', [...config.DEFAULT_PIO_ARGS, ...args], callback, cacheValid);
}
...
export function revealFolder(path) {
  ...
  runCommand(cmd, args, (code, stdout, stderr) => {
    if (code !== 0 && stderr) {
      notifyError(`Could not reveal a folder ${path}`, new Error(stderr));
    }
  });
}
