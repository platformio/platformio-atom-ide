...
Please report to https://github.com/platformio/platformio-atom-ide/issues`,
    dismissable: true
  });
  console.error(error);
}
...
export function getCoreVersion(callback) {
  runCommand('platformio', ['--version'], (code, stdout, stderr) => {
    ...
    const error = new Error('Failed to retrieve PIO Core version');
    error.stdout = stdout;
    error.stderr = stderr;
...
export function getCoreVersionAsync() {
  return new Promise ((resolve, reject) => {
    getCoreVersion((result, error) => {
      if (error) {
        reject(error);
      }
      else {
