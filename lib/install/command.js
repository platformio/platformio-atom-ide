...
import {CACHE_DIR, ENV_DIR, ENV_BIN_DIR} from '../constants';
...
export function uninstallPlatformIO() {
  const executable = path.join(ENV_BIN_DIR, 'pip');
  const result = child_process.spawnSync(executable, ['uninstall', '-y' ,'platformio']);
  if (0 !== result.status) {
    let title = 'PlaftormIO: Failed to uninstall PlatformIO!';
