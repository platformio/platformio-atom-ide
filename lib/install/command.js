      ...
      (state) => {  // Manage the package dependencies
        ...
        if (state.restored) {
          ...
          p = p.then(() => {
            ...
            return new Promise((resolve, reject) => {
              const child = child_process.spawn(apm, ['uninstall'].concat(packagesToRemove));
              child.on('close', (code) => {
                if (0 !== code) {
                  atom.notifications.addError(
                    `PlatformIO: Failed to uninstall stale dependencies.`,
                    {dismissable: true});
                    reject();
                }
                resolve();
        ...
        p = p.then(() => {
          ...
          return new Promise((resolve, reject) => {
            const child = child_process.spawn(apm, ['install'].concat(packagesToInstall));
            child.on('close', (code) => {
              if (0 !== code) {
                atom.notifications.addError(
                  `PlatformIO: Failed to install dependencies.`,
                  {dismissable: true});
                  reject();
              }
              resolve();
        ...
        p = p.then(() => {
          ...
          return new Promise((resolve, reject) => {
            const child = child_process.spawn(apm, ['upgrade', '--no-confirm'].concat(packagesToUpgrade));
            child.on('close', (code) => {
              if (0 !== code) {
                atom.notifications.addError(
                  `PlatformIO: Failed to upgrade dependencies.`,
                  {dismissable: true});
                  reject();
              }
              resolve();
        ...
        p = p.then(() => {
          ...
          for (let i = 0; i < packagesToEnable.length; i++) {
            activatePromise = activatePromise.then(() =>
              atom.package.activatePackage(packagesToEnable[i]));
          }
