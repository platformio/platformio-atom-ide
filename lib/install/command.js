...
function checkIfExamplesHaveToBeDownloaded(state) {
  return new Promise((resolve) => {
    fs.stat(path.join(config.CACHE_DIR, 'examples.tar.gz'), (err) => {
...
function getExamples(state) {
  const url = 'https://github.com/platformio/platformio-examples/tarball/develop';
  ...
  return new Promise(resolve => {
    ...
    .then((archivePath) => new Promise((resolve, reject) => {
      fs.readdir(archivePath, (err, files) => {
        ...
        fs.copy(path.join(archivePath, files[0]), folder, err => {
          if (err) {
            reject(err);
            return;
          }
...
function installVirtualenvIntoTemporaryDirectory(extractedPath) {
  return new Promise((resolve, reject) => {
    ...
    installProcess.on('error', onError);
    installProcess.on('close', (code) => {
      if (0 !== code) {
        onError(installProcess.stderr);
      }
