...
import {BASE_DIR, CACHE_DIR, DEPENDENCIES, ENV_DIR, ENV_BIN_DIR, STALE_DEPENDENCIES} from '../config';
  ...
  return Promise.resolve()
    ...
    .then(() => {
      return new Promise((resolve, reject) => {
        ...
        child.on('close', (code) => {
          if (0 !== code) {
            let title = 'PlatformIO: Failed to install PlatformIO!';
...
function installVirtualenvIntoTemporaryDirectory(extractedPath) {
  return new Promise((resolve, reject) => {
    ...
    installProcess.on('close', (code) => {
      if (0 !== code) {
        let title = 'PlatformIO: Unable to install the virtualenv.';
...
function makePenv(virtualenvRoot) {
  return new Promise((resolve, reject) => {
    ...
    if (-1 === virtualenvScript) {
      let title = 'PlatformIO: Cannot find the virtualenv.py script.';
    ...
    makeEnvProcess.on('close', (code) => {
      if (0 !== code) {
        let title = 'PlatformIO: Unable to create a virtualenv.';
        atom.notifications.addError(
...
function checkIfPlatformIOCanBeExecuted() {
  return new Promise((resolve, reject) => {
    ...
    pioVersionProcess.on('close', (code) => {
      if (0 !== code) {
        let title = 'PlatformIO tool is not available.';
