  ...
  if (useBuiltinPlatformIO()) {
    ...
    const tasks = [
      ...
      (state) => {  // Install the PlatformIO
        ...
        if (state.virtualenvCreated) {
          return new Promise((resolve) => {
            const args = ['install', 'https://github.com/platformio/platformio/archive/develop.zip'];
      ...
      (state) => {  // Manage the package dependencies
        const availablePackages = atom.packages.getAvailablePackageNames();
        let p = Promise.resolve();
        if (state.restored) {
          // Installing not for the first time
          p = p.then(() => {
            const packagesToRemove = STALE_DEPENDENCIES
              .filter((name) => availablePackages.indexOf(name) > 0);
            return Promise.all(packagesToRemove.map((packageName) => {
              return new Promise((resolve, reject) => {
                const child = child_process.spawn(apm, ['uninstall', packageName]);
                child.on('close', (code) => {
                  if (0 !== code) {
                    atom.notifications.addError(
                      `PlatformIO: Failed to uninstall ${packageName}`,
                      {dismissable: true});
                      reject();
                  }
                  resolve();
        ...
        p = p.then(() => {
          ...
          return Promise.all(packagesToInstall.map((packageName) => {
            return new Promise((resolve, reject) => {
              const child = child_process.spawn(apm, ['install', packageName]);
              child.on('close', (code) => {
                if (0 !== code) {
                  atom.notifications.addError(
                    `PlatformIO: Failed to install ${packageName}`,
                    {dismissable: true});
                    reject();
                }
                resolve();
        ...
        p = p.then(() => {
          ...
          return Promise.all(packagesToUpgrade.map((packageName) => {
            return new Promise((resolve, reject) => {
              const child = child_process.spawn(apm, ['upgrade', '--no-confirm', packageName]);
              child.on('close', (code) => {
                if (0 !== code) {
                  atom.notifications.addError(
                    `PlatformIO: Failed to upgrade ${packageName}`,
                    {dismissable: true});
                    reject();
                }
                resolve();
        ...
        p = p.then(() => {
          ...
          return Promise.all(packagesToEnable.map((name) => atom.package.activatePackage(name)));
        });
      ...
      (state) => {  // Adjust the toolbar position
        if (!state.restored) {
