...
export function command() {
  ...
  view.handleOpen = () => {
    let command;
    if (WIN32) {
      command = `platformio -f -c atom serialports monitor -p ${view.getPort()} -b ${view.getBaudrate()}`;
    } else {
      let fullPIOPath = path.join(ENV_BIN_DIR, 'platformio');
      if (DARWIN) {
        command = `script /dev/null "${fullPIOPath}" -f -c atom serialports monitor ` +
                  `-p ${view.getPort()} -b ${view.getBaudrate()}`;
      } else {
        command = `script -c "${fullPIOPath} -f -c atom serialports monitor ` +
