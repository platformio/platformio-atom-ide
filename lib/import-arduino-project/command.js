...
import {handleLibChanges, initializeProject, installPlatformsForBoards} from '../init/command';
  ...
  view.handleImport = () => {
    ...
    chain = chain.then(() => {
      ...
      if (useArduinoLibManager) {
        fs.remove(path.join(projectPath, 'lib'));
      }
    }).catch((reason) => {
    ...
    async function copyWholeProject() {
      ...
      function copyFilesIntoSubdir(copyPath) {
        return new Promise((resolve, reject) => {
          const dstDir = path.join(projectPath, path.basename(projectPath));
          try {
