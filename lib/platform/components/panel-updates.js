...
export default class PlatformInstalledPanel extends BasePanel {
  ...
  async onDidUpdateOnlyPackages(event) {
    event.stopPropagation();
    event.target.classList.add('btn-inprogress', 'disabled');
    try {
      const result = await runPlatformCommand('update', {
        extraArgs: ['--only-packages']
      });
      ...
      <div>
        <div className='block row'>
          ...
          <div className='col-xs-3 text-right'>
            <button onclick={ (e) => this.onDidUpdateOnlyPackages(e) } className='btn btn-primary icon icon-cloud-download inline-block-tight'>
              Update Packages
            </button>
          </div>
        </div>
