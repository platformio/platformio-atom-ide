...
export default class PlatformMain extends React.Component {
  ...
  onDidInstall(event) {
    ...
    this.props.installPlatform(
      `${this.props.data.name}@${this.versionSelect.value}`,
        ...
        <dl className='row inset-panel padded'>
          ...
          <dd className='col-xs-10 inline-buttons'>
            { this.props.data.frameworks.map(item => (
                <button key={ item.name } onClick={ () => this.props.showFramework(item.name) } className='btn btn-sm icon icon-tag inline-block-tight'>
                  { item.title }
                </button>
              )) }
          </dd>
