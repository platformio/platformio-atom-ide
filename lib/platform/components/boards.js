          ...
          { items.map(item => (
              <Tr>
                <Td column='id' value={ item.id }>
                  <span><a onClick={ () => atom.clipboard.write(item.id) }><span title='Copy to clipboard' className='icon icon-clippy'></span></a> <span className='inline-block highlight'>{ item.id }</span></span>
                </Td>
