    ...
    for (const pattern of patterns) {
      glob.sync(path.join(libDir, pattern)).forEach(item => {
        ...
        const name = dir
          ...
          .replace(new RegExp(`examples?${path.sep}`, 'i'), '');
...
class LibLibraryExamplesView extends EtchComponent {
  ...
  render() {
    return (
      <div>
        <ul style={ { display: !this.props.items.length ? 'block' : 'none' } } className='background-message text-center'>
          <li>
            No examples
          </li>
        </ul>
...
class LibLibraryExampleFilesView extends EtchComponent {
