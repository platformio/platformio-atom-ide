...
import LibInstallAdvancedModal from './install-advanced-modal';
...
export default class LibRegistryStatsPanel extends BasePanel {
  ...
  onDidPanelShow() {
    ...
    // reset previous view
    this.refs.registryStats.update({
      data: null
    });
    runLibraryCommand('stats', {
      extraArgs: ['--json-output']
    }).then(data => {
      // freeze state for 1 hour
      this.freezePanel();
      setTimeout(() => this.unfreezePanel(), 3600 * 1000);

      this.refs.registryStats.update({ data });
    });
  }
...
class LibRegistryStatsView extends BaseView {
  ...
  onDidInstall(event) {
    ...
    new LibInstallAdvancedModal().prompt().then(lib => {
  ...
  render() {
    return (
      <div className='lib-stats'>
        <div className='row'>
          ...
          <div className='col-xs text-right'>
            <div className='btn-group'>
              <button onclick={ () => this.onDidSearch('') } className='btn btn-primary icon icon-code'>
                All Libraries
              </button>
              <button onclick={ () => this.onDidRegister() } className='btn btn-primary icon icon-file-add'>
                Register
              </button>
              <button onclick={ (e) => this.onDidInstall(e) } className='btn btn-primary icon icon-cloud-download'>
                Advanced
              </button>
            </div>
          </div>
        ...
        { this.props.data && !this.props.data.updated ? (
          <ul className='background-message text-center'>
            <li>
              No Results
            </li>
