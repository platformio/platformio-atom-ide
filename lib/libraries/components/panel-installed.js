    ...
    return (
      <div>
        { this.props.storages.map((storage, storageIndex) => (
            <div>
              <h1 className='section-heading icon icon-file-directory'>{ storage.name } <span className='badge badge-medium'>{ this.getStatus(storageIndex) === LibInstalledView.status.LOADED ? this.getStorageItems(storageIndex).length : '' }</span></h1>
              ...
              { this.getStorageItems(storageIndex).map((item, index) => (
                  <LibInstalledStorageItem homebus={ this.props.homebus } data={ item } onuninstall={ () => this.onDidUninstall(storageIndex, index) } />
                )) }
            </div>
          )) }
      </div>
