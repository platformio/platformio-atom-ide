...
class LibBuiltInView extends EtchComponent {
  ...
  onDidToggleStorageList(event, storagePath) {
        ...
        { (this.props.items ? this.props.items : []).map(storage => (
            <div>
              <h1 className='section-heading icon icon-file-directory'>
                ...
                <span className='pull-right'><a onclick={ (e) => this.onDidToggleStorageList(e, storage.path) }><span className='icon icon-fold'></span></a></span>
              </h1>
