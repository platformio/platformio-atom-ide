...
import { expandFrameworksOrPlatforms } from '../../platform/util';
...
export default class LibLibraryPanel extends BasePanel {
    ...
    switch (typeof this.props.data) {
      case 'number':
        this.props.data = await runLibraryCommand(
          'show', {
            extraArgs: [parseInt(this.props.data), '--json-output']
          });
        break;
  ...
  async sanitizeRawManifest(data) {
    ...
    for (const key of ['platforms', 'frameworks']) {
      ...
      data[key] = await expandFrameworksOrPlatforms(key, data[key]);
    }
...
class LibLibrarySubmenu extends BaseView {
