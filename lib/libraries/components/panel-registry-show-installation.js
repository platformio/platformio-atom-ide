...
export default class LibRegistryShowInstallationPanel extends BasePanel {
  ...
  onDidVersionInstall(version) {
    this.props.homebus.emit('lib-install', `${this.props.data.id}@${version}`);
  }
    ...
    return (
      <div className='lib-installation'>
        ...
        <ul className='list-inline'>
          ...
          <li>
            <button onclick={ () => this.onDidVersionInstall(this.refs.versionSelect.value) } className='btn btn-primary icon icon-cloud-download'>
              Install
            </button>
          </li>
        </ul>
