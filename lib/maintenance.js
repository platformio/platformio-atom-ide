...
export function checkClang() {
  ...
  utils.runCommand('clang', ['--version'], code => {
    if (code === 0) {
      return localStorage.setItem('platformio-ide:clang-checked', '2');
    }
    atom.confirm({
      ...
      detailedMessage: 'PlatformIO IDE uses "Clang" for the Intelligent Code Completion.\n' +
        'Please install it and restart Atom, otherwise this feature will be disabled.',
      buttons: {
        'Install Clang': () => utils.openUrl('http://docs.platformio.org/page/ide/atom.html#clang-for-intelligent-code-completion'),
        'Remind Later': utils.doNothing,
        'Disable Code Completion': () => localStorage.setItem('platformio-ide:clang-checked', '2')
      }
    });
  });
}
