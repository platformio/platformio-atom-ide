...
import {getPythonExecutable, getIDEVersion, runAtomCommand, useBuiltinPlatformIO, getActiveProjectPath} from './utils';
...
export function updateOSEnviron() {
  ...
  process.env.PLATFORMIO_FORCE_COLOR = "true";
...
export function rebuildIndex() {
  ...
  if (!projectPath) {
    atom.notifications.addError('Please open the project directory.');
    return;
  }

  const iniPath = path.join(projectPath, 'platformio.ini');
  if (!fs.statSyncNoException(iniPath)) {
    atom.notifications.addError('Please initialize new project first.');
    return;
  }
  ...
  for (let section of configSections) {
    if (section.indexOf(envNamePrefix) === 0 && config[section].board) {
      const result = child_process.spawnSync(
        'platformio', ['init', '--ide', 'atom', '-b', config[section].board],
        {cwd: projectPath});
      if (0 === result.status) {
        atom.notifications.addSuccess('Index has ben successfully rebuilt.');
      } else {
        atom.notifications.addWarning('Failed to rebuild index.');
      }
      rebuildOccured = true;
      break;
    }
  ...
  if (!rebuildOccured) {
    atom.notifications.addWarning(
      'Rebuild operation has been skipped (empty project).');
  }
}
