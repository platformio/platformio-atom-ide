...
import {run as runInTerminal} from './terminal';
...
export function openTerminal() {
  const status = runInTerminal([]);
  if (0 !== status) {
    atom.notifications.addError('Unable to open the terminal', {
      detail: 'Make sure you have terminal-plus package installed.',
    });
  }
}

export function serialMonitor() {
  const status = runInTerminal(['platformio serialports monitor']);
  if (0 !== status) {
    atom.notifications.addError('Unable to run the Serial Monitor', {
      detail: 'Make sure you have terminal-plus package installed.',
    });
  }
}
...
export function serialPorts() {
  const status = runInTerminal(['platformio serialports list']);
  if (0 !== status) {
    atom.notifications.addError('Unable to list Serial Ports', {
      detail: 'Make sure you have terminal-plus package installed.',
    });
  }
}
