...
export function makeSurePlatformioInstalled() {
  if (useBuiltinPlatformIO()) {
    if (!fs.statSyncNoException(ENV_DIR)) {  // env does not exist.
      ...
      const result = child_process.spawnSync(pythonExecutable, ['--version']);
      if (0 !== result.status) {
      ...
      https.get('https://pypi.python.org/pypi/virtualenv/json', (res) => {
        ...
        res.on('end', () => {
          ...
          tarball.extractTarballDownload(downloadUrl, tmpFile.path, tmpDirPath, {}, (err, result) => {
            ...
            const installProcess = child_process.spawnSync(pythonExecutable, installArgs, installOptions);
            ...
            // Make a virtualenv
            const virtualenvScript = findFileByName('virtualenv.py', tmpDirInstalledPath);
            ...
            const makeEnvProcess = child_process.spawnSync(pythonExecutable, [virtualenvScript, ENV_DIR]);
    ...
    const pioVersionProcess = child_process.spawnSync(getPlatformIOExecutable(), PLATFORMIO_BASE_ARGS);
...
function installPlatformio() {
  ...
  const installResult = child_process.spawnSync(path.join(ENV_BIN_DIR, 'pip'), args);
