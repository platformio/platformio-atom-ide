...
import {getActiveProjectPath, getIDEVersion, getPythonExecutable, isPioProject, useBuiltinPlatformIO} from './utils';
import {rebuildIndex, rebuildIndexSync} from './init/command';
...
export function ensureProjectsInitedSync(projectPaths, force=false) {
...
export function handleLibChanges(projectPaths) {
  ...
  projectPaths.map(p => {
    ...
    const existingWatches = _allLibWatchers.get(p);
    ...
    const necessaryWatches = [
      ...
      {
        type: TYPE__DIR,
        path: path.join(p, 'lib'),
      }
    ];

    const warningMessage = 'Failed to get a global libraries directory.';
    try {
      const args = ['-c', 'from platformio import util; print util.get_home_dir()'];
      const child = child_process.spawnSync(getPythonExecutable(), args, {cwd: p});
      if (child.status === 0) {
        const globalLibDir = path.join(child.stdout.toString().trim(), 'lib');
