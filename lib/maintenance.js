...
import {ENV_BIN_DIR, WIN32, BASE_DIR, NO_ELIGIBLE_PROJECTS_FOUND} from './config';
...
export function handleLibChanges(projectPaths) {
  let Directory;
  try {
    Directory = require(path.join(process.resourcesPath, 'app.asar', 'node_modules', 'pathwatcher')).Directory;
  } catch(e) {
  ...
  const removedPaths = _currentPaths.filter(p => !projectPaths.contains(p));
  ...
  projectPaths.map(p => {
    ...
    if (!_allLibWatchers.has(p)) {
      _allLibWatchers.set(p, {
        local: {root: null, subdirs: new Map()},
        global: {root: null, subdirs: new Map()},
      });
    }
    ...
    let globalLibDir;
    if (process.env.PLATFORMIO_LIB_DIR) {
      globalLibDir = process.env.PLATFORMIO_LIB_DIR;
    } else {
      const config = ini.parse(fs.readFileSync(path.join(p, 'platformio.ini')).toString());
      globalLibDir = config && config.platformio && config.platformio.lib_dir;
    }
    if (!globalLibDir) {
      globalLibDir = '~/.platformio/lib';
    }
    if (globalLibDir.startsWith('~/')) {
      globalLibDir = process.env.HOME + globalLibDir.substring(1);
    }

    if (!projectLibDisposables.local.root) {
      const dir = new Directory(localLibDirPath);
      if (dir.existsSync()) {
        projectLibDisposables.local.root = dir.onDidChange(() => {
          // TODO: perform a rebuild only when new subdirectories added.
          // TODO: watch on subdirectories. When `src` directory created under subdirectories, it most probably
          // should be included into index as well.
          console.debug(`Local lib of ${p} changed. Rebuilding...`)
          rebuildIndex(p);
        });
      }
    ...
    console.debug('Global lib dir:', globalLibDir)
    if (!projectLibDisposables.global.root) {
      const dir = new Directory(globalLibDir);
      if (dir.existsSync()) {
        projectLibDisposables.global.root = dir.onDidChange(() => {
          // TODO: perform a rebuild only when new subdirectories added.
          // TODO: watch on subdirectories. When `src` directory created under subdirectories, it most probably
          // should be included into index as well.
          console.debug(`Global lib of ${p} changed. Rebuilding...`)
          rebuildIndex(p);
        });
      }
    }
...
export function clearLibChangeWatchers(paths) {
  ...
  // TODO: dispose everything from _allLibWatchers
}
