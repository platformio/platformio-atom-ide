...
export function setupActivationHooks() {
  const atomVersion = atom.getVersion();
  if (semver.satisfies(atomVersion, '<1.14.0')) {
    atom.confirm({
      message: `Your Atom ${atomVersion} is out of date. Update now for better performance and stability.`,
      buttons: {
        'Get it now': () => shell.openExternal('https://atom.io'),
        'Remind Later': () => {},
      }
    });
  }

  if (semver.satisfies(atomVersion, '<1.12.2')) {
    return;
  }
