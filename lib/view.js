...
export class FilteredList extends BaseView {

  constructor() {
    super(...arguments);

    this._filterSubscriptions = new CompositeDisposable();
    this._filterSubscriptions.add(
      this.filterEditor.onDidChange(::this.onDidFilterChange));
    this._onFilterTimeoutId = null;
  }

  get filterEditor() {
    throw new Error('FilteredList components must implement a `get filterEditor` method');
  }

  onDidFilter() {
    throw new Error('FilteredList components must implement a `onDidFilter` method');
  }

  onDidFilterChange() {
    if (this._onFilterTimeoutId) {
      clearTimeout(this._onFilterTimeoutId);
      this._onFilterTimeoutId = null;
    }
    this._onFilterTimeoutId = setTimeout(
      () => this.onDidFilter(this.filterEditor.getText()), 100);
  }

  destroy() {
    this._filterSubscriptions.dispose();
...
export class PanelSubmenuView extends BaseView {
