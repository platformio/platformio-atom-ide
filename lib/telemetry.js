...
export default class Telemetry {
  ...
  async fetchCid() {
    ...
    try {
      this._cid = await new Promise(function(resolve, reject) {
  ...
  async send(hittype, params) {
    ...
    try {
      params = Object.assign(params, Telemetry.defaultParams);
      params.t = hittype;
      params.cid = await this.fetchCid();
      params.an = await this.fetchAppName();
      request.post('https://www.google-analytics.com/collect').form(params);
      return true;
    } catch (err) {
      return false;
    }
