...
export function setBusyRegistry(registry) {
  __BUSY_REGISTRY = registry;
}

export function getBusyRegistry() {
...
export async function rebuildIndex(projectPath) {
  ...
  function startBusy () {
    __BUSY_REGISTRY && __BUSY_REGISTRY.begin(
      'platformio.index-rebuild-' + projectPath,
      `PlatformIO: Rebuilding C/C++ Project Index for ${projectPath}`);
  }

  function stopBusy () {
    __BUSY_REGISTRY && __BUSY_REGISTRY.end(
      'platformio.index-rebuild-' + projectPath);
  }
}
...
export function handleLibChanges(projectPaths) {
  ...
  projectPaths.map(p => {
    ...
    try {
      ...
      const child = child_process.spawnSync(utils.getPythonExecutable(), args, {cwd: p});
      if (child.status === 0) {
