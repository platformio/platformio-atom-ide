...
export function getRecentProjects(filterQuery) {
  return db.projects
    .orderBy('lastSeen')
    .filter((project) => filterQuery? project.path.indexOf(filterQuery) !== -1 : true)
    .reverse()
...
export function openPIOHome(force=false) {
  if (force || atom.config.get('platformio-ide.showHomeScreen')) {
