...
import { BasePanel } from './panels-view';
...
export default class WelcomePanel extends BasePanel {
  ...
  updateRecentProjects() {
    getRecentProjects().toArray()
      .then((items) => {
        this.refs.recentProjects.update({items: items});
  ...
  render() {
    return (
      <div className='home-welcome-view'>
        <div className='block head'>
          ...
          <div className='toggle'>
            <label className='input-label text-smaller'>
              <input ref='openOnStartup' className='input-toggle' type='checkbox' onchange={this.onDidOpenOnStartup} checked={true} />
              &nbsp; Open at startup
            </label>
          </div>
        </div>
        ...
        <RecentProjectsView
          ref='recentProjects'
          onselect={::this.onDidRecentProjectSelect}
          onremove={::this.onDidRecentProjectRemove} />
      </div>
    );
  }
