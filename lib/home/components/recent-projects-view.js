...
export default class RecentProjectsView extends EtchComponent {
  ...
  getItems() {
    ...
    return fuzzaldrin.filter(
      this.props.items, this.refs.filterEditor.getText(), {
        key: 'path'
      }
    );
  }
        ...
        <div className='select-list'>
          <ol className='list-group'>
            { this.getItems().map((item) => (
                <RecentProjectItem item={ item } onselect={ () => this.props.onselect(item) } onremove={ () => this.props.onremove(item) } />
              )) }
          </ol>
        </div>
      </atom-panel>
    ...
    return (
      <li className='two-lines'
        ...
        <div className='primary-line icon icon-file-directory'>
          { path.basename(this.props.item.path) }
        </div>
