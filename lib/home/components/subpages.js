      ...
      <div>
        <div className='page-submenu btn-group btn-group-lg'>
          { this.props.routes.map(item => item.label && (
              <button onClick={ () => goTo(this.context.router.history, item.path, item.pathState) } className={ ['btn', 'icon', `icon-${item.icon}`, getActiveRoute(this.props.routes, this.context.router.route.location) === item ? 'selected' : ''].join(' ') }>
                { item.label }
              </button>
            )) }
        </div>
